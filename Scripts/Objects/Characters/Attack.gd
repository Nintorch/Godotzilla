...
func state_init() -> void:
	char.animation_player.connect("animation_finished", _on_animation_finished)

func _process(delta: float) -> void:
	walk_state.walk(delta)

func use(type: GameCharacter.Attack) -> void:
	char.set_state(GameCharacter.State.ATTACK)
	...
	match type:
		GameCharacter.Attack.PUNCH:
			variation = !variation
			if variation:
				char.animation_player.play("Punch1")
			else:
				char.animation_player.play("Punch2")
			char.get_sfx("GodzillaPunch").play()
		GameCharacter.Attack.KICK:
			variation = !variation
			if variation:
				char.animation_player.play("Kick1")
			else:
				char.animation_player.play("Kick2")
			char.get_sfx("GodzillaPunch").play()
